...
export function createIntegrationNote(input: {
  ...
  return [
    ...
    `Params: ${input.params}`,
    `Error Handling: ${input.errorHandling}`,
    `Rate Limits: ${input.rateLimits}`,
    `SDK Install: ${input.sdkInstall}`,
    `Minimal Example: ${input.minimalExample}`,
    "Sources:",
    ...input.sources.map((source) => `- ${source.url} (${source.location}, fetched ${source.fetchedAt})`)
  ].join("\n");
}
